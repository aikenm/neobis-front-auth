...
import '../../styles/PasswordRecoveryPage/RecoveryPasswordForm.css';
...
function RecoveryPasswordForm() {
  ...
  const onSubmit = (data) => {
    const userEmail = JSON.parse(localStorage.getItem("recoveryEmail"));
    const payload = { email: userEmail, password: data.password };
    console.log(payload);
  ...
  return (
    <div className='password-form-wrapper'>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='password-form'>
            ...
            <h2 className='password-title'>Сброс пароля</h2>
            ...
            <button type="submit" className='form-button' disabled={!allCriteriaMet}>Сбросить</button>
        </form>
    </div>
  );
}
